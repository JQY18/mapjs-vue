<template>
  <div class="location-management">
    ...
    >
      <el-form :model="locationForm" :rules="rules" label-width="100px">
    ...
    <!-- 图片预览模态框 -->
    <image-preview v-model:show="previewVisible" :image-url="previewImage" />
...
const fetchLocations = async () => {
  try {
    const { data } = await locationApi.getLocations(currentAdmin.value.locationId);
    if (data.code === 1) {
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm("确认删除该地点？此操作不可恢复！", "警告", {
...
const handleSubmit = async () => {
  try {
    ...
    // 只在有新图片时添加图片文件
    if (
      locationForm.value.imageFile &&
      (!editingLocation.value ||
        locationForm.value.image !== editingLocation.value.image)
    ) {
